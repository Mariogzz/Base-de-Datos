...
CREATE PROCEDURE EmpleadosEdad @Edad tinyint
AS 
SELECT * FROM Empleado WHERE Edad=@Edad;

EXEC EmpleadosEdad @Edad=18;

CREATE PROCEDURE ComidaPrecios @Precio money
AS 
SELECT * FROM Comida WHERE Costo=@Precio;

EXEC ComidaPrecios @Precio=$70;

CREATE PROCEDURE BebidaPrecios @Precio money
AS 
SELECT * FROM Bebida WHERE Costo=@Precio;

EXEC BebidaPrecios @Precio=$50;

CREATE PROCEDURE RestaurantesLoc @Local varchar(50)
AS 
SELECT * FROM Restaurant WHERE Direccion=@Local;

EXEC RestaurantesLoc @Local='Apodaca huinala';
 
select * from Restaurant

CREATE PROCEDURE Mesas @Capacidad tinyint
AS 
SELECT * FROM Mesa WHERE Capacidad=@Capacidad;

EXEC Mesas @Capacidad=4;

CREATE FUNCTION Propina(@total money) RETURNS money
AS
BEGIN
DECLARE @Propina money
SET @Propina=(@total*.10)
RETURN @Propina
END

SELECT dbo.Propina(565) AS PropinaAMesero
GO

CREATE PROCEDURE dbo.Costos @Costo money 
AS 
BEGIN
SELECT * FROM Comida
WHERE Costo=@Costo
