...
CREATE TABLE [dbo].[Prueba](
	[ID_Prueba] [tinyint] NOT NULL,
	[Letra] [varchar](50) NOT NULL,
	)

INSERT INTO Prueba(ID_Prueba,Letra)
	VALUES(1,'a');
INSERT INTO Prueba(ID_Prueba,Letra)
	VALUES(2,'b');
...
SELECT Nombre_Comida AS Platillo
FROM Comida;

SELECT (Nombre + ' ' + ApellidoMat + ' ' + ApellidoPat) AS NombreCompleto
FROM Empleado;

SELECT Nombre AS Restaurante
FROM Restaurant;

SELECT Nombre_Bebida AS Refrescos
FROM Bebida;

SELECT Nombre_Comida AS Platillo, Costo AS Precio
FROM Comida

SELECT Empleado.ID_Empleado,Puesto.ID_Puesto,Empleado.Nombre
FROM Empleado
INNER JOIN Puesto ON Empleado.ID_Empleado=Puesto.ID_Puesto;
...
SELECT (Empleado.Nombre + ' ' + Empleado.ApellidoMat) AS NombreEmpleado,Puesto.Sueldo
FROM Empleado
RIGHT JOIN Puesto ON Empleado.ID_Puesto= Puesto.ID_Puesto;
...
SELECT COUNT(ID_Puesto) AS CantidadDePuestos
FROM Puesto;

SELECT AVG(Capacidad) AS CapacidadPromedio
FROM Mesa;

SELECT Nombre_Comida,MIN(Costo) AS Precio
FROM Comida
GROUP BY Nombre_Comida, Costo;

SELECT SUM(Stock) AS StockTotal
FROM Bebida
GROUP BY Stock;

SELECT Nombre, MAX(Edad) AS Edad
FROM Empleado 
